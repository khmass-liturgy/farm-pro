// ─── 문자 발송 (뿌리오 SMS API 중계) ───────────────────────────────────────
// 뿌리오는 토큰 발급 시 미리 등록해둔 발신 IP만 허용한다({"code":"3003",
// "description":"invalid ip"}). 이 Edge Function은 서버리스라 나가는 IP가
// 매번 달라질 수 있어 뿌리오를 직접 호출할 수 없다. 그래서 실제 뿌리오 호출은
// 고정 IP를 가진 별도 서버(sms-relay/, 예: Oracle Cloud 무료 VPS)가 담당하고,
// 이 함수는 "로그인 확인 → 그 서버에 전달"만 한다.
//
//   브라우저 → 이 Edge Function(로그인 확인) → sms-relay(고정 IP) → 뿌리오
//
// 뿌리오 인증키(계정/토큰)는 이제 이 함수에 전혀 없다 — sms-relay 서버의
...
const LMS_BYTE_LIMIT = 2000; // 장문(LMS) 바이트 한도(뿌리오 기준) — 여기서도 먼저 걸러 sms-relay를 불필요하게 깨우지 않는다
